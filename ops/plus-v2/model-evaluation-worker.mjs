...
export function createModelEvaluationWorker(options){
  const {tenantId,identities,loadPolicy,servicesFor}=options;
  if(typeof servicesFor!=='function'||typeof identities?.resolvePrincipal!=='function')fail('EVALUATION_WORKER_CONFIGURATION_INVALID');
  ...
  function targets(){access.assertConfigured();const value=loadPolicy().evaluationJobs;
  ...
  async function principal(target){const p=await identities.resolvePrincipal(target.workerId);
    if(p.id!==target.workerId||p.tenantId!==tenantId||!p.roles.includes('plus_governance_worker'))fail('EVALUATION_WORKER_IDENTITY_FORBIDDEN');
    if(!targets().some(t=>t.key===target.key&&t.workerId===target.workerId))fail('EVALUATION_WORKER_CONFIGURATION_STALE');return p;
  }
  function service(){const services=servicesFor();services.assertConfigured();
    if(!services.evaluationJobs)fail('EVALUATION_WORKER_SERVICE_UNAVAILABLE');return services.evaluationJobs;
  }
    ...
    try{
      ...
      try{
        if(row.operation==='RECONCILE_EXHAUSTED'){
          ...
          state={...state,status:'DEGRADED',lastOutcome:result.status,lastError:'EVALUATION_JOB_ATTEMPTS_EXHAUSTED',processed:state.processed+1};return;
        }
        if(row.operation!=='CLAIM')fail('EVALUATION_WORKER_DISCOVERY_INVALID');
        ...
        if(result.id!==row.id||result.status!=='SUCCEEDED')fail('EVALUATION_WORKER_RESULT_UNCONFIRMED');
        ...
        state={...state,status:'DEGRADED',lastError:'EVALUATION_WORKER_OPERATION_FAILED'};
        try{
          ...
          if(TERMINAL.has(current.status)){
            state={...state,status:current.status==='SUCCEEDED'?'IDLE':'DEGRADED',lastOutcome:current.status,
              lastError:current.status==='SUCCEEDED'?null:'EVALUATION_WORKER_TERMINAL_FAILURE',processed:state.processed+1};
        ...
        }catch{state={...state,lastOutcome:'RECOVERY_REQUIRED',lastError:'EVALUATION_WORKER_RECOVERY_REQUIRED'};}
      }
    }catch{state={...state,status:'DEGRADED',lastError:'EVALUATION_WORKER_DISCOVERY_FAILED'};}
  }
  return {
