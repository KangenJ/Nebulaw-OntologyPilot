...
import {createNativeDecisionJobs} from '../public-plus/native-decision-jobs-ui.js';

// Executes shipped handlers with explicit DOM/HTTP adapters, not browser QA.
const command={mode:'DECIDE',input:{key:'task.model',evaluationId:'score',evaluationVersion:1,decision:'APPROVE',reason:'Sensitive reason',requestKey:'original-key'}};
function fixture({saved=new Map()}={}){
  ...
  let p={id:'owner',tenantId:'native',roles:['model_owner']},row={id:'job-1',key:'task.model',version:1,status:'PENDING',attempts:0,mode:'DECIDE',commandHash:'a'.repeat(64),recordedDecision:null,qualification:'NOT_CHECKED',predictionReady:false,executionAuthorized:false};
  ...
  Object.defineProperty($('#decision-jobs'),'innerHTML',{get:()=>html,set:v=>html=v});
  const api=async(path,epoch,body)=>{calls.push({path,body:structuredClone(body)});if(hold){const waiting=hold;hold=undefined;await waiting;}if(unknown)throw Error('UNKNOWN_RESPONSE');
    ...
    if(path.endsWith('/lookup'))return {schema:'plus-decision-job-lookup-v1',key:'task.model',item:empty?null:item,readOnly:true,absenceIsNotCancellation:true,predictionReady:false,executionAuthorized:false};
    if(path.includes('?'))return {schema:'plus-decision-job-index-v1',key:'task.model',items:[item],readOnly:true,predictionReady:false};return item;
  };
  const run=fn=>{if(busy)return;busy=true;last=fn(1).catch(e=>error=e).finally(()=>busy=false);return last;};
  const ui=createNativeDecisionJobs({document:{querySelector:$,querySelectorAll:()=>[]},api,run,isBusy:()=>busy,getPrincipal:()=>p,getStorage:()=>storage});ui.render();
  return {ui,saved,calls,$,html:()=>html,settle:()=>last,error:()=>error,setActor:v=>p=v,hold:v=>hold=v,unknown:v=>unknown=v,corrupt:()=>corrupt=true,empty:()=>empty=true,denyStorage:()=>deniedStorage=true,
    finish:()=>{row={...row,version:3,status:'SUCCEEDED',attempts:1,recordedDecision:{id:'decision',version:1,decision:'APPROVE',contentHash:'b'.repeat(64)}};}};
}
test('short owner submit persists only original lookup metadata, not material or credentials; terminal lookup releases it',async()=>{
  const f=fixture();assert.equal(f.ui.submit(command),true);await f.settle();assert.equal(f.calls[0].path,'/learning/decision-jobs');
  ...
  f.finish();await f.ui.lookup();assert.equal(f.saved.size,0);assert.match(f.html(),/SUCCEEDED/);assert.match(f.html(),/不代表当前资格、模型生效或在线许可/);
...
test('reload recovers exact original intent with a read-only lookup and never automatically reposts',async()=>{
  ...
  reopened.empty();await reopened.ui.lookup();assert.deepEqual(reopened.calls[0],{path:'/learning/decision-jobs/lookup',body:{key:'task.model',requestKey:'original-key'}});
...
test('changed identity/reset discards late responses, and forged readiness is rejected without releasing recovery metadata',async()=>{
  ...
  const g=fixture();g.ui.submit(command);await g.settle();g.corrupt();await g.ui.lookup();assert.match(g.html(),/INVALID_DECISION_JOB_RECEIPT/);assert.equal(g.saved.size,1);
});
