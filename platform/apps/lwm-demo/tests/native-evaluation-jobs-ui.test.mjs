...
import {createNativeEvaluationJobs} from '../public-plus/native-evaluation-jobs-ui.js';

// Executes shipped handlers with explicit DOM/HTTP adapters, not browser QA.
const command={mode:'EVALUATE',input:{key:'task.model',protocolId:'protocol',executionId:'fit',validationDatasetIds:['validation'],requestKey:'original-key'}};
function fixture({saved=new Map()}={}){
  ...
  let p={id:'owner',tenantId:'native',roles:['trainer']},row={id:'job-1',key:'task.model',version:1,status:'PENDING',attempts:0,mode:'EVALUATE',commandHash:'a'.repeat(64),recordedEvaluation:null,qualification:'NOT_CHECKED',predictionReady:false,executionAuthorized:false};
  ...
  Object.defineProperty($('#evaluation-jobs'),'innerHTML',{get:()=>html,set:v=>html=v});
  const api=async(path,epoch,body)=>{calls.push({path,body:structuredClone(body)});if(hold){const waiting=hold;hold=undefined;await waiting;}if(unknown)throw Error('UNKNOWN_RESPONSE');
    ...
    if(path.endsWith('/lookup'))return {schema:'plus-evaluation-job-lookup-v1',key:'task.model',item:empty?null:item,readOnly:true,absenceIsNotCancellation:true,predictionReady:false,executionAuthorized:false};
    if(path.includes('?'))return {schema:'plus-evaluation-job-index-v1',key:'task.model',items:[item],readOnly:true,predictionReady:false};return item;
  };
  const run=fn=>{if(busy)return;busy=true;last=fn(1).catch(e=>error=e).finally(()=>busy=false);return last;};
  const ui=createNativeEvaluationJobs({document:{querySelector:$,querySelectorAll:()=>[]},api,run,isBusy:()=>busy,getPrincipal:()=>p,getStorage:()=>storage});ui.render();
  return {ui,saved,calls,$,html:()=>html,settle:()=>last,error:()=>error,setActor:v=>p=v,hold:v=>hold=v,unknown:v=>unknown=v,corrupt:()=>corrupt=true,empty:()=>empty=true,denyStorage:()=>deniedStorage=true,
    finish:()=>{row={...row,version:3,status:'SUCCEEDED',attempts:1,recordedEvaluation:{id:'score',version:1,contentHash:'b'.repeat(64)}};}};
}
test('short trainer submit persists only original lookup metadata, not material or credentials; terminal lookup releases it',async()=>{
  const f=fixture();assert.equal(f.ui.submit(command),true);await f.settle();assert.equal(f.calls[0].path,'/learning/evaluation-jobs');
  ...
  f.finish();await f.ui.lookup();assert.equal(f.saved.size,0);assert.match(f.html(),/SUCCEEDED/);assert.match(f.html(),/不是模型批准、生效或在线许可/);
...
test('reload recovers exact original intent with a read-only lookup and never automatically reposts',async()=>{
  ...
  reopened.empty();await reopened.ui.lookup();assert.deepEqual(reopened.calls[0],{path:'/learning/evaluation-jobs/lookup',body:{key:'task.model',requestKey:'original-key'}});
  assert.match(reopened.html(),/不代表取消/);assert.equal(reopened.saved.size,1);
});
test('same-page retry retains the exact command; storage errors and non-trainer identities cannot post',async()=>{
...
test('changed identity/reset discards late responses, and forged readiness is rejected without releasing recovery metadata',async()=>{
  ...
  f.ui.reset();f.setActor({id:'other',tenantId:'native',roles:['trainer']});f.ui.render();release();await lookup;
  assert.equal(f.error().discarded,true);assert.doesNotMatch(f.html(),/job-1/);assert.equal(f.saved.size,1);
  const g=fixture();g.ui.submit(command);await g.settle();g.corrupt();await g.ui.lookup();assert.match(g.html(),/INVALID_EVALUATION_JOB_RECEIPT/);assert.equal(g.saved.size,1);
});
