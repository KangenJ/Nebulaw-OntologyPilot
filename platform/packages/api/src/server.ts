...
async function main(): Promise<void> {
  ...
  const nativeDemo = process.env['LWM_NATIVE_ENABLED'] === 'true';
  if (nativeDemo && (!isDev || !process.env['LWM_AUTH_FILE'] || !process.env['CEL_EVALUATOR_URL'] || process.env['POSTGRES_URL'])) {
    throw new Error('Native demo requires local mode, LWM_AUTH_FILE and canonical CEL_EVALUATOR_URL; PostgreSQL migration is not yet validated');
  }
  ...
  if (nativeDemo) {
    const moduleUrl = new URL('../../../apps/lwm-demo/src/native-storage.mjs', import.meta.url).href;
    const { createNativeStorage } = await import(moduleUrl);
    storage = createNativeStorage(process.env['LWM_NATIVE_DATABASE_PATH'] ?? 'var/lwm/native-platform.sqlite');
    logger.info('Storage: durable native SPI demo adapter (objects, links, history; no extension aggregate)');
  ...
  if (nativeDemo) {
    let current;
    try { current = await storage.getSchema(bootCtx); } catch { /* First boot. */ }
    if (current) {
      const upgraded = structuredClone(current);
      for (const type of spiSchema.objectTypes) {
        const existing = upgraded.objectTypes.find(item => item.name === type.name);
        if (!existing) upgraded.objectTypes.push(type);
        else for (const field of type.properties) if (!existing.properties.some(item => item.name === field.name)) existing.properties.push(field);
      }
      for (const link of spiSchema.linkTypes) if (!upgraded.linkTypes.some(item => item.name === link.name)) upgraded.linkTypes.push(link);
      if (JSON.stringify(upgraded) !== JSON.stringify(current)) { upgraded.version = current.version + 1; await storage.applySchema(bootCtx, upgraded); }
    } else await storage.applySchema(bootCtx, spiSchema);
  } else await storage.applySchema(bootCtx, spiSchema);
  ...
  if (seedManifests.length > 0 && !nativeDemo) {
    let seededObjects = 0;
    let seededLinks = 0;
  ...
  if (!isDev && process.env['OPENFGA_URL'] && process.env['OPENFGA_STORE_ID']) {
    ...
    logger.warn(nativeDemo ? 'Authorization: generic development authorizer unreachable; native facade enforces tenant, roles and independent approval' : 'Authorization: allow-all stub (development mode)');
  } else {
  ...
  if (!nativeDemo) assertFgaModelCoverage(fgaModelJson, schema, isDev);
  ...
  if (nativeDemo) {
    const moduleUrl = new URL('../../../apps/lwm-demo/src/native-platform.mjs', import.meta.url).href;
    const { nativeActionRoles, plusRoles } = await import(moduleUrl);
    security = { async checkPermission(actor, action, params, ctx) {
      const role = action === 'NativePlusCommand' && typeof params['operation'] === 'string' && Object.hasOwn(plusRoles, params['operation']) ? plusRoles[params['operation']] : nativeActionRoles[action];
      return { allowed: ctx.tenantId === 'lwm-demo' && typeof role === 'string' && actor.roles.includes(role), reason: 'Native demo action role and tenant required' };
    } };
  } else if (!isDev) {
  ...
  const auditStore = (storage instanceof PostgresStorageProvider)
    ...
    : nativeDemo ? (storage as StorageProvider & { auditStore: MemoryAuditStore }).auditStore : new MemoryAuditStore();
  ...
  if (storage instanceof PostgresStorageProvider) {
    ...
    logger.info('Audit: native SQLite append-only demo store (post-commit; not external WORM)');
  } else {
  ...
  // ── Express + HTTP Server ──
  const app = express();
  // Native demo exposes only its authenticated integration facade until the
  // generic REST/GraphQL/FGA surfaces receive equivalent governance validation.
  // Do not let the platform's generic development allow-all CRUD bypass actions.
  if (nativeDemo) app.use((req, res, next) => {
    if (req.path.startsWith('/api/lwm/') || req.path === '/health' || req.path === '/ready') { next(); return; }
    res.status(403).json({ error: { code: 'NATIVE_SURFACE_DISABLED', message: 'Use the authenticated native demo facade' } });
  });
  ...
  const subscriptionManager = new SubscriptionManager({
    ...
    authenticate: async (connectionParams) => {
      if (nativeDemo) return { authenticated: false, error: 'Native demo subscriptions are not enabled' };
  ...
  // Optional LWM extension: shared durable command backend, never UI-local state.
  if (nativeDemo) {
    const moduleUrl = new URL('../../../apps/lwm-demo/src/native-platform.mjs', import.meta.url).href;
    const { installNativePlatform } = await import(moduleUrl);
    await installNativePlatform(app, { storage, objectManager, schema, actionExecutor, manifestRegistry, auditStore, cel, isDev });
  } else if (process.env['LWM_EXTENSION_ENABLED'] === 'true') {
    const extensionUrl = new URL('../../../apps/lwm-demo/platform-extension.mjs', import.meta.url).href;
    const { installLwmExtension } = await import(extensionUrl);
    await installLwmExtension(app, {
      pool: storage instanceof PostgresStorageProvider ? storage.pool : undefined,
      isDev,
      authenticate: (request: import('express').Request) => extractUser(request, authenticator, false),
    });
  }
  ...
  await new Promise<void>((resolve) => {
    ...
    if (isDev && !['127.0.0.1', '::1', 'localhost'].includes(host)) {
      throw new Error('Development allow-all services must bind loopback');
    }
  ...
  if (nativeDemo) {
    logger.info(`  Native facade: http://localhost:${PORT}/api/lwm/ (authenticated); generic GraphQL/REST/FHIR/subscriptions disabled`);
    return;
  }
