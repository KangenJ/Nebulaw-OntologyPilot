...
export async function loadDomainPacks(
  ...
  const manifestRegistry: ManifestRegistry = {
    get(actionName: string) {
      // Retired LWM demo YAMLs are reference schema only. They used scalar gates
      // and must not provide a second, weaker REST/GraphQL execution path.
      const retiredLwmActions = new Set(['ApproveTransition', 'RejectTransition', 'RequestEvidence', 'CompleteInvestigationTask', 'PromoteModel', 'RollbackModel', 'HoldModel']);
      if (manifests.some(pack => pack.name === 'lwm-demo') && retiredLwmActions.has(actionName)) return undefined;
      // The new manifests are available only in the validated native demo
      // entrypoint. Generic development mode uses an allow-all CEL stub and
      // must never accidentally activate these actions during pack discovery.
      if (['NativeImportMatter', 'NativeVerifyObservation', 'NativeProposeTransition', 'NativeReviewTransition', 'NativePlusCommand'].includes(actionName)
        && process.env['LWM_NATIVE_ENABLED'] !== 'true') return undefined;
      return actionManifests.get(actionName);
    },
  };
